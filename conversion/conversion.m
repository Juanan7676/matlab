function[coords] = conversion(var1,var2,lat,modo)
    ...
    elseif modo=="E"
        if v1 < pi
          ...
          A = asin(sin(pi/2-v2)*sin(v1)/sin(XZ))*180/pi;
          coords = [minuto(180-A) minuto(h)];
        else
          XZ = acos(cos(pi/2-l)*cos(pi/2-v2)+sin(pi/2-l)*sin(pi/2-v2)*cos(2*pi-v1));
          h = 90 - XZ*180/pi;
       
          A = asin(sin(pi/2-v2)*sin(2*pi-v1)/sin(XZ))*180/pi + 180;
          coords = [minuto(A) minuto(h)];
       end
    end
